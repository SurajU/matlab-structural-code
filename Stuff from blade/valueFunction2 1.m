...
drawsLambdaNextTemp = simulateLambda2(lambdaPreviousPeriod,uniformDrawsLambda,...
    parLambdaProcess,t);
...
for j = 1:length(squeeze(capRArray(1,:,1)))
    mOptimal(ceil(j/step):end,j) = squeeze(lambdaArray(ceil(j/step):end,1,1)) + omega;
    v(ceil(j/step)+1:end,j) = -(j-1)*(5/step) + (t<=12).*discountFactor^abs(t-12)*(omega/2) + (t<=11).*discountFactor^abs(t-11)*(omega/2)+...
